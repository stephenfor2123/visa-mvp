...
> 文档版本:v2.0
> 发布日期:2026-06-11
> 前置基线:`V0_AI签证需求文档.md`(2026-05 草案,11 章纯工具版骨架)
> 升级基线:`V2_资料整理.md`(T1 整理,581 行素材包,本文件在此基础上正式成稿)
...
| iOS App | Flutter 3.x,iOS 14+ | **移动主入口(先做 iOS)** | 摄像头/麦克风/相册/推送 | 权限运行时申请,TestFlight 内部测试 | **V2 先行** |
| 微信小程序 | uni-app x(同 Flutter 业务逻辑) + WebView 壳 | 轻量入口,免下载 | 受限功能(无后台保活) | 见 §2.3.3 | **V2** |
| 后台(Admin Web) | Vue 3 + Element Plus,独立 SPA | 内部管理员(超管/运维) | 配置管理、订单查询、审计 | 仅 Web,无移动端 | **V2** |
| Android App | Flutter 3.x,minSdk 26(Android 8.0) | 移动端补充 | 摄像头/麦克风/相册 | 权限运行时申请 | **V3+ 推后** |
...
#### 3.3.6 N1 扫描引导页 React 组件(完整关键方法,无截断)

```tsx
// web/src/features/scan/ScanGuidePage.tsx
// React 18 + Vite + TypeScript · 关键方法完整
import React, { useRef, useEffect, useState, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { ScanRepository } from '@/api/scan';
import type { ScanResult } from '@/types/scan';

interface ScanGuidePageProps {
  materialType: 'passport' | 'id_card' | 'household' | 'employment' | 'bank' | 'flight' | 'hotel';
  onCaptured: (result: ScanResult) => void;
}
...
export const ScanGuidePage: React.FC<ScanGuidePageProps> = ({ materialType, onCaptured }) => {
  const videoRef = useRef<HTMLVideoElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const streamRef = useRef<MediaStream | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [hint, setHint] = useState(HINTS[materialType] || HINTS.passport);
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      alert('Camera permission required. Please grant access in browser settings.');
    }
  }, []);
  ...
  }, [isProcessing, onCaptured, stopCamera]);
  ...
  return (
    <div className="scan-guide">
      <header className="scan-guide__header">
        <button onClick={() => navigate(-1)} className="btn-back">Back</button>
        <h2>Scan Document</h2>
      </header>
      <div className="scan-guide__viewport">
        <video ref={videoRef} playsInline muted className="scan-guide__video" />
        <canvas ref={canvasRef} hidden />
        <div className="scan-guide__frame" />
        <div className="scan-guide__hint">{hint}</div>
      </div>
      <footer className="scan-guide__footer">
        <button
...
### 4.6 支付服务(Payment Service · Mock 实现)

#### 4.6.1 功能(V2 范围 = 完整 Mock)

| 功能 | 说明 | 真实/ Mock |
|---|---|---|
| 创建支付 | 订单提交后生成 payment 记录 | Mock |
| 拉起支付 | 前端跳到"模拟支付页"输卡号(随便输)/输 4242 测试卡号 | Mock UI |
| 异步回调 | Mock 服务器主动推 `paid` 状态(模拟 1-3s 延迟) | Mock |
| 查询支付 | GET /payments/{id} 返回状态 | Mock |
| 拒签退费 | 订单标 rejected 时自动触发 refund | Mock |
| **真实通道预留** | 接口设计留 `channel` 字段(alipay/wxpay/stripe),V3+ 接入 | 预留 |

> **V2 决策(2026-06-11 用户确认)**:支付完全 Mock,前端跳"模拟支付页"演示流程,接口契约按真实通道设计,后期切换只改 `channel` + adapter,业务逻辑零改动。

#### 4.6.2 Mock 支付前端页 UX(2 步)

```
[订单详情页] → 点击"立即支付"
   ↓
[模拟支付页 /mock-pay/{payment_id}]
   ├─ 标题:模拟支付(MVP 测试)
   ├─ 卡号输入:默认填 "4242 4242 4242 4242" (Stripe 测试卡)
   ├─ 有效期 / CVV:可任意填
   ├─ 支付金额:从接口取
   ├─ [确认支付] 按钮 → POST /payments/{id}/mock-confirm
   └─ 1.5s 后跳转回订单详情,状态 = paid
```
...
CREATE TABLE payments (
    ...
    currency        VARCHAR(8) NOT NULL DEFAULT 'USD',
    channel         VARCHAR(32) NOT NULL DEFAULT 'mock',  -- mock / alipay / wxpay / stripe
    status          VARCHAR(16) NOT NULL DEFAULT 'pending',  -- pending / paid / refunded / failed
    external_txn_id VARCHAR(128),                          -- mock_confirm_xxx
    paid_at         TIMESTAMPTZ,
    refunded_at     TIMESTAMPTZ,
    refund_amount   NUMERIC(10,2),
    mock_meta       JSONB,                                 -- mock 渠道额外信息(测试卡号/Mock延迟)
    created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE refunds (
    ...
    order_id        BIGINT NOT NULL REFERENCES orders(id),
    reason          VARCHAR(64) NOT NULL,                  -- rejection / user_cancel / admin
...
-- 渠道 adapter 注册表(后期接真实支付时用)
CREATE TABLE payment_adapters (
    id              SMALLSERIAL PRIMARY KEY,
    code            VARCHAR(32) NOT NULL UNIQUE,           -- mock / alipay / wxpay / stripe
    name            VARCHAR(64) NOT NULL,
    config          JSONB,                                 -- app_id / merchant_id / public_key 等
    enabled         BOOLEAN NOT NULL DEFAULT TRUE,
    is_mock         BOOLEAN NOT NULL DEFAULT FALSE
);
-- 种子数据
INSERT INTO payment_adapters (code, name, is_mock) VALUES
  ('mock',    'Mock Pay(MVP)',        TRUE),
  ('alipay',  '支付宝(预留,V3+ 接入)', FALSE),
  ('wxpay',   '微信支付(预留,V3+ 接入)', FALSE),
  ('stripe',  'Stripe(预留,V3+ 接入)', FALSE);
```

#### 4.6.4 Mock 支付流程

```
[订单详情页] --"立即支付"--> POST /api/v2/payments {order_id}
   ↓
[API] 创建 payment(status=pending, channel=mock) → 返回 payment_id
   ↓
[前端] 跳转 /mock-pay/{payment_id} 模拟支付页
   ↓
[用户] 输测试卡号 → 点"确认支付"
   ↓
[前端] POST /api/v2/payments/{id}/mock-confirm
   ↓
[API] Celery 任务延时 1.5s → 写 status=paid / paid_at=now → 触发订单状态机 submitted
   ↓
[前端] 轮询 payment status / 或用 WebSocket → 跳回订单详情
```
...
- [ ] OCR 拍摄引导页边框对齐、闪光灯、倒计时正常
- [ ] 护照 OCR 字段准确率 ≥ 95%(100 张样本)
...
- [ ] OCR 准确率 ≥ 95% / 验证码识别率 ≥ 90% / RPA 提交成功率 ≥ 95%
