...
修整 #3 — B-W10-4 producer 写完 SDK 调用但测试 `test_payment_stripe_stub.py` 的 `_FakeSettings` 只 mock 了 `stripe_secret_key` 1 个字段, 实际 `__init__` 要读 3 个字段。**B-W11-2 修整**: test `_FakeSettings` 补齐 2 字段 (已修, 1/1 PASS in 0.92s)。
...
修整留 W12+ 实现: 加 `payment_channel: Literal["mock", "stripe"]` 配置, `get_payment_provider()` 根据 channel 返回 Mock 或 Stripe 实例。
...
修整 #4 (本任务): V2.1 凭据接入准备 recipe 文档化 (本文件), V2 阶段零凭据, 修整 #3 (test 1/1 PASS)。

---

**V2.1 接入判定**: 修整 #4 完成 (recipe 文档 + macOS Keychain 路径), V2 仍 Mock (修整 #3 test PASS), V2.1 真接留 W12+ Story。
