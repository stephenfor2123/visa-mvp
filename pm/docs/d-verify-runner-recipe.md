# D-VERIFY-RUNNER 真值 grep recipe (失察 修整)

> 修整背景: 2026-06-12 W6b 17:54 + W8 21:49 + W9 22:53 + W9 23:10 + W10 23:55 共 5 次失察 — D Coordinator 跑 4 必查时漏查 1 P0 (deliverable.md / A_WORKLOG.md / screenshots sha256 / pytest 缺任一)。根因: D-PRODUCER-EVIDENCE-NO-DEFER rule 实战 — D 不信 producer 报告, 必跑 4 必查实测, 但 D 仍漏查。

## 1. 4 必查 — wire-level recipe (W12+ 必跑)

每个 producer 任务收口时, D verifier (或 D 自身) 必跑以下 4 步, 任一 FAIL 即整体 FAIL:

### Step 1 — 截图 sha256 distinct
```bash
# 真值 grep recipe — 反 W6b A-W6-4 教训
...
```bash
grep "<TASK_ID>" backend/WORKLOG.md   # 或 frontend/C_WORKLOG.md
...
W6b 17:54 漏查 `deliverable.md` 不存在 — D override_accept 时只 Read 了 producer 主报告, 没跑 step 2。
...
- **C-W11-3 (本 sprint W11 sub-task 3)**: 写 `tools/d-verify-runner.sh` (≥ 50 行 bash) 跑 4 必查, 输出 PASS/FAIL + 数字。W12+ 每 plan_yaml verify_prompt 必带 "run D-VERIFY-RUNNER" 强制约束。
...
## 5. W12+ verify_prompt 必带行

每个 plan_yaml task 的 verify_prompt 末尾必加:
```yaml
# W12+ D-VERIFY-RUNNER 强制约束
verify_prompt_extras: |
  Run D-VERIFY-RUNNER before accept:
    bash tools/d-verify-runner.sh <TASK_ID> frontend/web/screenshots/ outputs/<TASK_ID>/deliverable.md backend/WORKLOG.md
