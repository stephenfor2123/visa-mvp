...
## 2026-07-22 — 定价模块动态预览（仅本地，未发布）

- 参考用户提供的 Flowbite 定价对比截图，重做 `web/src/components/PricingSection.vue` 展示层。
- 新增四国动态选择、使馆费/平台服务费/退款保障三列决策区、推荐列悬浮反馈、入场动画、按钮反馈及横向费用对比表。
- 保留 `usePlatformPricing` 后台定价与促销数据来源，未修改支付、订单或后台定价逻辑。
- 适配移动端及 `prefers-reduced-motion`，没有引入 Flowbite/Tailwind 或复制 Flowbite Pro 源码。
- 验证：Vite 前端编译通过；`PaymentCheckout.test.ts` 1/1 通过。完整 `npm run build` 在既有 SEO 预渲染的本地 4173 预览服务启动超时处停止。
- **发布状态：仅本地预览；未部署、未推送、未提交、线上无变化。**

## 2026-07-22 — 防 AI 复制现状审查（仅记录，未实施）

- 发现 `web/public/robots.txt` 当前主动允许 GPTBot、ClaudeBot、Google-Extended、anthropic-ai、Applebot-Extended 与 PerplexityBot，并公开 `llms-full.txt`；这有利于 GEO，但与限制 AI 训练/复制的目标冲突。
- 建议优先级：区分搜索型与训练型爬虫 → 将核心签证字段映射/诊断规则后端化 → 内存限流迁移 Redis 并收紧代理 IP 信任 → 开启 Cloudflare Bot Fight Mode / Block AI Bots / AI Labyrinth → 给 Vercel 前端补 CSP/HSTS 等响应头。
- 已确认的安全项：生产包无 Source Map；Git 未跟踪真实 `.env`；生产环境关闭 API docs/OpenAPI；PWA 不缓存 `/api/*`；已有 CORS、鉴权与基础限流。
- **本次仅审查和记录，没有修改 robots、防火墙、接口、部署或线上配置。**
