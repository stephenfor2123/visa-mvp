...
test.describe('G. 兼容性 / Compat (W19)', () => {
  ...
  test('G10.1 375 视口下切 en 语言 -> html lang=en', async ({ browser }) => {
    ...
    await page.goto(`${BASE}/home`, { waitUntil: 'domcontentloaded' })
    await page.waitForLoadState('networkidle')
    // W19-2: redesigned from 4 inline pills to dropdown — open trigger, click item
    await page.locator('.lang-switch__trigger').first().click()
    await page.waitForTimeout(80)
    await page.locator('.lang-switch__item', { hasText: 'EN' }).first().click()
  ...
  test('G10.2 1280 视口下切 id 语言 -> html lang=id-ID', async ({ browser }) => {
    ...
    await page.locator('.lang-switch__trigger').first().click()
    await page.waitForTimeout(80)
    await page.locator('.lang-switch__item', { hasText: 'ID' }).first().click()
    await page.waitForTimeout(300)
    const lang = await page.evaluate(() => document.documentElement.lang)
    expect(lang).toBe('id-ID')
    await ctx.close()
